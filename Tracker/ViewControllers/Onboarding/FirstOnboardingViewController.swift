...
final class FirstOnboardingViewController: UIViewController {
    ...
    private lazy var backgroundImageView: UIImageView = {
        ...
        if let image = UIImage(named: "onboardingScreen1") {
            imageView.image = image
        } else {
            print("\(#file):\(#line)] \(#function) Ошибка загрузки изображения onboardingScreen1")
        }
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        setupViews()
        print("\(#file):\(#line)] \(#function) Первый экран онбординга загружен")
    }
